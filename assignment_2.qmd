...
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc_pruned.A2 \
--king-cutoff 0.0884 \
...
plink2 --bfile ./02_activities/data/gwa.qc_unrelated_pruned.A2 \
--pca 20 \
...
```{r manhattan-plot, echo=FALSE, fig.width=10, fig.height=6}
library(data.table)
library(qqman)

# Read GWAS association results
assoc <- fread("./02_activities/data/gwa.qc_assoc.A2.PHENO1.glm.linear")

# Filter additive model
assoc_add <- assoc[TEST == "ADD"]

# Rename columns for qqman
setnames(assoc_add, c("#CHROM","POS","ID","P"), c("CHR","BP","SNP","P"))
...
```{r qq-plot, echo=FALSE, fig.width=6, fig.height=6}
# QQ plot
qq(assoc_add$P, main="GWAS QQ Plot")
```
